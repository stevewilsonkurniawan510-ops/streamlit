...
class ComponentManifestHandler:
    ...
    def process_manifest(
        ...
        for comp_config in manifest.components:
            component_name = f"{base_name}.{comp_config['name']}"
            ...
            if asset_dir:
                ...
                # Resolve absolute asset root path and enforce containment within package_root
                asset_root = (package_root / asset_dir).resolve()

                if not asset_root.exists() or not asset_root.is_dir():
                    raise StreamlitAPIException(
                        f"Declared asset_dir '{asset_dir}' for component '{component_name}' "
                        f"does not exist or is not a directory under package root '{package_root}'."
                    )

                # Ensure the resolved path (following symlinks) remains inside package_root
